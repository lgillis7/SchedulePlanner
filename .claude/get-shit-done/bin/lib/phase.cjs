...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  if (fs.existsSync(roadmapPath)) {
    ...
    // Table format: | Phase | Milestone | Plans | Status | Date |
    // $1 = phase + milestone cells (preserved), $2/$3 = remaining pipes
  ...
  // Find next phase — check both directories AND roadmap
  let nextPhaseNum = null;
  let nextPhaseName = null;
  let isLastPhase = true;

  // 1. Check phase directories first
  try {
    const entries = fs.readdirSync(phasesDir, { withFileTypes: true });
    const dirs = entries.filter(e => e.isDirectory()).map(e => e.name).sort((a, b) => comparePhaseNum(a, b));
  ...
  // 2. If no next directory found, check roadmap for unplanned phases
  if (isLastPhase && fs.existsSync(roadmapPath)) {
    try {
      const rmContent = fs.readFileSync(roadmapPath, 'utf-8');
      // Match unchecked phase checkboxes: - [ ] **Phase N: Name**
      const phaseCheckboxes = [...rmContent.matchAll(/-\s*\[\s*\]\s*\*\*Phase\s+(\d+[A-Z]?(?:\.\d+)*)[\s:]+([^*]+)\*\*/gi)];
      for (const m of phaseCheckboxes) {
        const candidateNum = m[1];
        const candidateName = m[2].replace(/[:\s]+$/, '').trim();
        if (comparePhaseNum(candidateNum, phaseNum) > 0) {
          nextPhaseNum = candidateNum;
          nextPhaseName = candidateName.replace(/\s+/g, '-').toLowerCase();
          isLastPhase = false;
          break;
        }
      }
