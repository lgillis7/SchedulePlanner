...
**Cross-check `is_last_phase`:** Do NOT blindly trust this value. Before reporting "last phase" or "milestone complete", verify by checking the ROADMAP.md for unchecked phase checkboxes (`- [ ] **Phase N:**`) in the current milestone. If any exist with a phase number greater than the current phase, `is_last_phase` is wrong — override it to `false` and use the first unchecked phase as `next_phase`.
